...
// @version      9.3
// @description  Всё в одном + автообновление с GitHub
// @author       You
// @match        https://crm.o3team.ru/*
// @grant        none
// @updateURL    https://raw.githubusercontent.com/thatsblake/ozon-crm-helper/main/content.js
// @downloadURL  https://raw.githubusercontent.com/thatsblake/ozon-crm-helper/main/content.js
// ==/UserScript==
...
(function() {
    ...
    async function checkForUpdates() {
        try {
            ...
            if (latest !== CURRENT_VERSION) {
                console.log(`🔄 Доступно обновление: ${CURRENT_VERSION} → ${latest}`);
                
                // Скачиваем новый скрипт
                const scriptResp = await fetch(SCRIPT_URL + '?t=' + Date.now());
                const newScript = await scriptResp.text();
                
                // Сохраняем в localStorage
                localStorage.setItem('ozon_crm_update_' + latest, newScript);
                localStorage.setItem('ozon_crm_pending_update', latest);
                
                showUpdateNotification(latest);
            } else {
                console.log(`✅ Версия ${CURRENT_VERSION} — актуальна`);
            }
        } catch(e) {
            console.log('⚠️ Не удалось проверить обновление:', e.message);
        }
    }

    function showUpdateNotification(version) {
        // Проверяем, не показывали ли уже
        if (localStorage.getItem('ozon_crm_update_shown_' + version)) return;
        localStorage.setItem('ozon_crm_update_shown_' + version, '1');
        ...
        notif.innerHTML = `
            ...
            <div style="color:${T.textMuted};margin-bottom:10px;">
                Доступна новая версия помощника!<br>
                Нажмите «Обновить» чтобы перезагрузить страницу с новым кодом.
            </div>
        ...
        document.getElementById('notif-update').onclick = () => {
            localStorage.removeItem('ozon_crm_update_shown_' + version);
            localStorage.removeItem('ozon_crm_pending_update');
            location.reload();
        };
    }

    // Применяем обновление, если есть сохранённое
    (function applyPendingUpdate() {
        const pending = localStorage.getItem('ozon_crm_pending_update');
        if (pending) {
            const code = localStorage.getItem('ozon_crm_update_' + pending);
            if (code) {
                ...
                setTimeout(() => {
                    ...
                    n.style.cssText = `position:fixed;top:70px;right:20px;background:${T.bg};border:1px solid ${T.green};border-radius:8px;padding:10px;z-index:99999999;color:${T.text};font-size:12px;`;
                    n.textContent = `✅ Обновлено до версии ${pending}`;
    ...
    function loadSettings() {
        ...
        try {
            const saved = JSON.parse(localStorage.getItem('ozon_crm_settings'));
            if (saved) {
                if (!saved.stats) saved.stats = defaults.stats;
                if (!saved.stats.sessionStart) saved.stats.sessionStart = Date.now();
                if (!saved.hotkeys) saved.hotkeys = defaults.hotkeys;
                if (!saved.templates || !saved.templates.length) saved.templates = defaults.templates;
                if (saved.mode === undefined) saved.mode = 'paraphrase';
                return saved;
            }
        } catch (e) {}
        return defaults;
    ...
    async function askAI(messages) {
        const r = await fetch(API_URL, { method: 'POST', headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${GITHUB_TOKEN}` }, body: JSON.stringify({ model: 'gpt-4o-mini', messages, temperature: 0.5, max_tokens: 500 }) });
    ...
    document.addEventListener('mouseup', function() {
        setTimeout(() => {
            ...
            if (text && text.length > 10) showSelectionPopup(text, sel);
    ...
    function showTemplatePopup(templateId) {
        ...
        const p = document.createElement('div');
        p.id = 'template-popup';
        p.style.cssText = `position:fixed;bottom:80px;right:500px;width:300px;background:${themes[settings.theme].bg};border:2px solid ${themes[settings.theme].accent};border-radius:12px;padding:12px;z-index:9999999;box-shadow:0 0 30px rgba(255,107,157,0.3);font-family:'Segoe UI',sans-serif;color:${themes[settings.theme].text};font-size:12px;line-height:1.4;`;
        p.innerHTML = `
            <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px;">
                <span style="color:${themes[settings.theme].accent};font-weight:600;font-size:13px;">🧩 ${tpl.name}</span>
                <button id="popup-close" style="background:none;border:none;color:${themes[settings.theme].textMuted};cursor:pointer;font-size:14px;">✕</button>
            </div>
            <div style="background:${themes[settings.theme].inputBg};border-radius:6px;padding:6px;margin-bottom:6px;border:1px solid ${themes[settings.theme].inputBorder};">${tpl.template}</div>
            <button id="popup-insert" style="width:100%;background:linear-gradient(135deg,${themes[settings.theme].accent},${themes[settings.theme].accent2});color:white;border:none;padding:7px;border-radius:6px;cursor:pointer;font-size:12px;font-weight:600;">📩 Вставить</button>`;
        document.body.appendChild(p);
        document.getElementById('popup-insert').onclick = function() { if (smartPasteToChat(tpl.template)) { hideTemplatePopup(); showStatus('✅ Вставлено!'); lastHash = ''; } };
        document.getElementById('popup-close').onclick = hideTemplatePopup;
        templatePopupVisible = true;
    }
    function hideTemplatePopup() { const p = document.getElementById('template-popup'); if (p) p.remove(); templatePopupVisible = false; }
    ...
    function createMainUI() {
        ...
        header.innerHTML = `<span id="header-title">✦ Помощник</span>
            <div style="display:flex;gap:4px;align-items:center;">
                <button id="mode-toggle" style="background:rgba(255,255,255,0.2);border:1px solid rgba(255,255,255,0.3);color:white;cursor:pointer;font-size:11px;padding:3px 8px;border-radius:4px;">💬 Чат</button>
        ...
        body.innerHTML = `
            ...
            <div id="paraphrase-mode">
                ...
                <div style="display:flex;gap:6px;margin:6px 0;">
                    ...
                    <button id="greeting-toggle" style="flex:1;padding:5px 8px;border-radius:6px;cursor:pointer;font-size:11px;font-weight:500;border:1px solid ${T.green};background:${settings.greetingEnabled ? T.green : 'transparent'};color:${settings.greetingEnabled ? 'white' : T.red};">${settings.greetingEnabled ? '✨ Приветствие' : '🚫 Приветствие'}</button>
                </div>
            ...
            <div id="calculator-mode" style="display:none;">
                <div style="background:${T.inputBg};border:1px solid ${T.inputBorder};border-radius:8px;padding:8px;">
                    ...
                    <button id="calc-copy" style="width:100%;margin-top:6px;padding:6px;border-radius:6px;border:1px solid ${T.accent2};background:${T.primaryBg};color:${T.primaryText};cursor:pointer;font-size:11px;">📋 Копировать</button>
                    <button id="calc-paste" style="width:100%;margin-top:4px;padding:6px;border-radius:6px;border:1px solid ${T.accent};background:transparent;color:${T.accent};cursor:pointer;font-size:11px;">📩 В чат</button>
                </div>
            </div>
            <div id="panel-templates" class="panel" style="display:none;margin-top:8px;border-top:1px solid ${T.inputBorder};padding-top:6px;">
                ...
                <div id="add-template-form" style="display:none;margin-top:6px;border-top:1px solid ${T.inputBorder};padding-top:6px;">
                    ...
                    <textarea id="tpl-prompt" placeholder="Описание для ИИ (как поймёт)" style="width:100%;min-height:30px;padding:5px;border:1px solid ${T.inputBorder};border-radius:6px;font-size:11px;outline:none;background:${T.inputBg};color:${T.text};resize:vertical;margin-bottom:4px;"></textarea>
            ...
            <div id="panel-history" class="panel" style="display:none;margin-top:8px;border-top:1px solid ${T.inputBorder};padding-top:6px;">
                <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px;"><span style="font-size:12px;color:${T.textMuted};">📚 ИСТОРИЯ</span><button id="btn-clear-history" style="background:none;border:none;color:${T.red};cursor:pointer;font-size:11px;">Очистить</button></div>
            ...
            <div id="panel-settings" class="panel" style="display:none;margin-top:8px;border-top:1px solid ${T.inputBorder};padding-top:6px;">
                <div style="font-size:12px;color:${T.textMuted};margin-bottom:6px;">⚙️ НАСТРОЙКИ</div>
                ...
                <details><summary style="font-size:11px;color:${T.textMuted};cursor:pointer;">⌨️ Хоткеи</summary><div style="margin-top:4px;display:flex;flex-direction:column;gap:3px;font-size:11px;">${['paraphrase|⟳','retry|🔄','copyFromChat|📋','pasteToChat|📩','toggleGreeting|✨','quickFriendly|😊','quickProfessional|💼','quickShort|✂️','quickPolite|🙏'].map(x=>{const[k,l]=x.split('|');return `<div style="display:flex;justify-content:space-between;align-items:center;font-size:11px;"><span style="color:${T.textMuted};">${l}</span><input type="text" id="hk-${k}" value="${settings.hotkeys[k]||''}" style="width:50px;padding:3px;border-radius:4px;border:1px solid ${T.inputBorder};background:${T.inputBg};color:${T.text};text-align:center;font-size:11px;outline:none;"></div>`;}).join('\n')}<button id="btn-save-hotkeys" style="margin-top:4px;width:100%;padding:5px;border-radius:6px;border:1px solid ${T.accent};background:${T.accent};color:white;cursor:pointer;font-size:11px;">💾 Сохранить</button></div></details>
            </div>`;
        ...
        toggleBtn.style.cssText = `position:fixed;bottom:20px;right:20px;width:48px;height:48px;background:linear-gradient(135deg,${T.accent},${T.accent2});color:white;border:none;border-radius:50%;font-size:20px;cursor:pointer;box-shadow:0 0 20px ${T.shadow};z-index:999998;display:flex;align-items:center;justify-content:center;transition:transform 0.3s;`;
        ...
        function calc() {
            ...
            if (cur.length > 15) cur = parseFloat(cur).toExponential(5);
            upd();
        }
        ...
        function acm(r, t) {
            ...
            const d = document.createElement('div'); d.style.cssText = `margin-bottom:8px;padding:6px 8px;border-radius:8px;font-size:12px;line-height:1.4;`;
            if (r === 'user') { d.style.cssText += `background:${T.primaryBg};border:1px solid ${T.primaryBorder};text-align:right;`; d.innerHTML = `<div style="color:${T.text};">${t}</div>`; }
            else { d.style.cssText += `background:${T.resultBg};border:1px solid ${T.inputBorder};`; d.innerHTML = `<div style="color:${T.accent};font-size:10px;margin-bottom:2px;">🤖</div><div style="color:${T.text};">${t}</div>`; }
            cmsg.appendChild(d); cmsg.scrollTop = cmsg.scrollHeight;
        }
        ...
        csnd.onclick = async function() {
            ...
            const ld = document.createElement('div'); ld.style.cssText = `color:${T.loadingText};font-size:11px;padding:4px 8px;margin-bottom:4px;`; ld.textContent = '⏳'; cmsg.appendChild(ld); cmsg.scrollTop = cmsg.scrollHeight;
            try { const r = await askAI([{ role: 'system', content: 'Ты помощник.' }, ...chatHistory.filter(m => m.role !== 'system').slice(-10).map(m => ({ role: m.role, content: m.text })), { role: 'user', content: t }]); ld.remove(); acm('assistant', r); } catch(e) { ld.remove(); acm('assistant', '❌ ' + e.message); }
        };
        ...
        function openEditForm(template) {
            ...
            document.getElementById('tpl-prompt').value = template.prompt;
            document.getElementById('tpl-text').value = template.template;
            document.getElementById('tpl-edit-id').value = template.id;
            document.getElementById('btn-save-tpl').textContent = '💾 Обновить';
        }
        function renderTemplates() {
            const l = document.getElementById('templates-list');
            if (!l) return;
            if (!settings.templates.length) { l.innerHTML = '<div style="color:' + T.textMuted + ';font-size:11px;text-align:center;padding:10px;">Нет шаблонов</div>'; return; }
            ...
            `).join('');
            document.querySelectorAll('.tpl-toggle').forEach(b => { b.onclick = function() { const idx = parseInt(this.dataset.i); settings.templates[idx].enabled = !settings.templates[idx].enabled; saveSettings(); renderTemplates(); }; });
            ...
            document.querySelectorAll('.tpl-name').forEach(el => { el.onclick = function() { const id = this.dataset.id; const tpl = settings.templates.find(t => t.id === id); if (tpl) openEditForm(tpl); }; });
        }
        document.getElementById('btn-add-template').onclick = function() {
            const form = document.getElementById('add-template-form');
            if (form.style.display === 'block') { form.style.display = 'none'; }
            else { form.style.display = 'block'; document.getElementById('tpl-name').value = ''; document.getElementById('tpl-prompt').value = ''; document.getElementById('tpl-text').value = ''; document.getElementById('tpl-edit-id').value = ''; document.getElementById('btn-save-tpl').textContent = '💾 Сохранить'; }
        };
        ...
        document.getElementById('btn-save-tpl').onclick = function() {
            ...
            if (!name || !prompt || !text) { alert('Заполните все поля'); return; }
            if (editId) { const idx = settings.templates.findIndex(t => t.id === editId); if (idx !== -1) { settings.templates[idx].name = name; settings.templates[idx].prompt = prompt; settings.templates[idx].template = text; showStatus('✅ Шаблон обновлён'); } }
            else { settings.templates.push({ id: 'c_' + Date.now(), name, prompt, template: text, enabled: true }); showStatus('✅ Шаблон добавлен'); }
            saveSettings(); renderTemplates(); document.getElementById('btn-cancel-tpl').click();
        };
        ...
        document.getElementById('btn-submit').onclick = async function() {
            ...
            try {
                const p = { professional: 'Перепиши в деловом.', friendly: 'Перепиши дружелюбно.', short: 'Сократи.', polite: 'Перепиши вежливо.', fix: 'Исправь ошибки.', original: 'Перефразируй.' };
            ...
            finally { document.getElementById('paraphrase-loading').style.display = 'none'; btn.disabled = false; btn.textContent = '⟳'; }
        };
        ...
        function renderHistory() {
            ...
            if (!l) return;
            if (!history.length) { l.innerHTML = '<div style="color:'+T.textMuted+';font-size:11px;text-align:center;padding:15px;">Пусто</div>'; return; }
            l.innerHTML = history.slice(0,10).map(i => `<div style="background:${T.inputBg};border-radius:6px;padding:7px 8px;margin-bottom:4px;cursor:pointer;border:1px solid ${T.inputBorder};font-size:11px;" onclick="document.getElementById('paraphrase-input').value='${i.text.replace(/'/g,"\\'").replace(/"/g,'&quot;').replace(/\n/g,'\\n')}';showStatus('✅');"><div style="display:flex;justify-content:space-between;color:${T.textMuted};font-size:10px;margin-bottom:3px;"><span>${i.type}</span><span>${i.date}</span></div><div style="color:${T.text};overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">${i.text.substring(0,80)}${i.text.length>80?'...':''}</div></div>`).join('');
        }
    ...
    // Проверка обновлений через 10 секунд после загрузки
