...
// @version      10.7
// @description  Кнопки + обновление + релизы
// @author       thatsblake
...
(function() {
    ...
    async function checkUpdates() {
        try {
            ...
            if (updateVersion) {
                if (btn) btn.style.animation = 'blink 1.5s infinite';
                showNotif(updateVersion);
            } else {
                if (btn) btn.style.animation = 'none';
                showStatus('✅ Версия актуальна (v' + CURRENT_VERSION + ')');
                // Показываем кнопку "Все релизы"
                setTimeout(() => {
                    showNotif(null);
                }, 1000);
            }
    ...
    function showNotif(v) {
        document.getElementById('upd-notif')?.remove();
        
        const n = document.createElement('div');
        n.id = 'upd-notif';
        n.style.cssText = 'position:fixed;top:20px;right:20px;width:340px;z-index:99999999;animation:slideIn 0.3s ease;';
        
        if (v) {
            // Есть обновление
            n.innerHTML = `
                <div style="background:rgba(22,22,22,0.96);backdrop-filter:blur(24px);border:1px solid rgba(255,255,255,0.06);border-radius:16px;padding:20px;box-shadow:0 20px 60px rgba(0,0,0,0.5);">
                    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px;">
                        <div style="display:flex;align-items:center;gap:12px;">
                            <div style="width:40px;height:40px;background:linear-gradient(135deg,#eab308,#f59e0b);border-radius:12px;display:flex;align-items:center;justify-content:center;font-size:18px;">✦</div>
                            <div>
                                <div style="font-weight:600;font-size:14px;color:#fff;">Обновление ${v}</div>
                                <div style="color:rgba(255,255,255,0.45);font-size:12px;">Доступна новая версия</div>
                            </div>
                        </div>
                        <button id="notif-close" style="background:none;border:none;color:rgba(255,255,255,0.35);cursor:pointer;font-size:18px;">✕</button>
                    </div>
                    <button id="notif-download" style="width:100%;background:linear-gradient(135deg,#eab308,#f59e0b);color:#fff;border:none;padding:12px;border-radius:10px;cursor:pointer;font-size:13px;font-weight:500;">⬇️ Скачать обновление</button>
                </div>`;
        } else {
            // Нет обновления — показываем кнопку "Все релизы"
            n.innerHTML = `
                <div style="background:rgba(22,22,22,0.96);backdrop-filter:blur(24px);border:1px solid rgba(255,255,255,0.06);border-radius:16px;padding:20px;box-shadow:0 20px 60px rgba(0,0,0,0.5);">
                    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px;">
                        <div style="display:flex;align-items:center;gap:12px;">
                            ...
                            <div>
                                <div style="font-weight:600;font-size:14px;color:#fff;">Версия актуальна</div>
                                <div style="color:rgba(255,255,255,0.45);font-size:12px;">v${CURRENT_VERSION}</div>
                            </div>
                        </div>
                        <button id="notif-close" style="background:none;border:none;color:rgba(255,255,255,0.35);cursor:pointer;font-size:18px;">✕</button>
                    </div>
                    <button id="notif-releases" style="width:100%;background:linear-gradient(135deg,#6366f1,#8b5cf6);color:#fff;border:none;padding:12px;border-radius:10px;cursor:pointer;font-size:13px;font-weight:500;">📦 Все релизы на GitHub</button>
                </div>`;
        }
        ...
        const downloadBtn = document.getElementById('notif-download');
        if (downloadBtn) {
            downloadBtn.onclick = () => {
                n.remove();
                window.open(`https://github.com/${GITHUB_USER}/${REPO_NAME}/releases`, '_blank');
                showStatus('📦 Открыт GitHub. Скачайте ZIP и обновите расширение');
            };
        }
        
        const releasesBtn = document.getElementById('notif-releases');
        if (releasesBtn) {
            releasesBtn.onclick = () => {
                n.remove();
                window.open(`https://github.com/${GITHUB_USER}/${REPO_NAME}/releases`, '_blank');
    ...
    setTimeout(() => {
        ...
        header.innerHTML = `
            <div style="display:flex;align-items:center;gap:10px;">
                ...
                <span id="header-title" style="font-size:15px;">Помощник</span>
            </div>
            <div style="display:flex;gap:4px;align-items:center;">
                <button id="mode-toggle" style="display:flex;align-items:center;gap:4px;background:${T.card};border:1px solid ${T.border};color:${T.muted};cursor:pointer;font-size:12px;padding:6px 12px;border-radius:8px;"><span>💬</span><span>Чат</span></button>
                <button id="calc-toggle" style="background:none;border:none;color:${T.muted};cursor:pointer;font-size:18px;padding:6px;">🧮</button>
                <button id="check-update-btn" style="background:none;border:none;color:${T.muted};cursor:pointer;font-size:18px;padding:6px;" title="Проверить обновления">🔄</button>
                <div style="width:1px;height:22px;background:${T.border};margin:0 6px;"></div>
                <button class="panel-btn" data-p="templates" style="background:none;border:none;color:${T.muted};cursor:pointer;font-size:18px;padding:6px;">🧩</button>
                <button class="panel-btn" data-p="stats" style="background:none;border:none;color:${T.muted};cursor:pointer;font-size:18px;padding:6px;">📊</button>
                <button class="panel-btn" data-p="history" style="background:none;border:none;color:${T.muted};cursor:pointer;font-size:18px;padding:6px;">📚</button>
                <button class="panel-btn" data-p="settings" style="background:none;border:none;color:${T.muted};cursor:pointer;font-size:18px;padding:6px;">⚙️</button>
                <div style="width:1px;height:22px;background:${T.border};margin:0 6px;"></div>
                <button id="main-minimize" style="background:none;border:none;color:${T.text};cursor:pointer;font-size:18px;padding:6px 8px;opacity:0.8;" title="Свернуть">—</button>
                <button id="main-close" style="background:none;border:none;color:${T.red};cursor:pointer;font-size:18px;padding:6px 8px;opacity:0.8;" title="Закрыть">✕</button>
            </div>`;
        ...
        // HTML интерфейса (как в твоей версии)
        body.innerHTML = `
            <div id="status-message" style="display:none;font-size:12px;color:${T.accent};margin-bottom:10px;text-align:center;padding:10px;background:${T.card};border:1px solid ${T.border};border-radius:10px;animation:fadeIn 0.2s ease;"></div>
            ...
            <div id="paraphrase-mode">
                <div style="background:${T.card};border:1px solid ${T.border};border-radius:12px;padding:14px;margin-bottom:10px;">
                    <textarea id="paraphrase-input" style="width:100%;min-height:70px;padding:12px;border:1px solid ${T.border};border-radius:10px;font-size:13px;resize:vertical;outline:none;background:${T.bg};color:${T.text};font-family:inherit;" placeholder="Введите текст для перефразирования..."></textarea>
                </div>
                <div style="display:flex;gap:8px;margin-bottom:10px;">
                    <button id="btn-copy-from-chat" style="flex:1;padding:8px 12px;border-radius:8px;font-size:12px;cursor:pointer;background:${T.card};border:1px solid ${T.border};color:${T.text};">📋 Из чата</button>
                    <button id="btn-retry-last" style="flex:1;padding:8px 12px;border-radius:8px;font-size:12px;cursor:pointer;background:${T.card};border:1px solid ${T.border};color:${T.text};">🔄 Последнее</button>
                ...
                <div style="background:${T.card};border:1px solid ${T.border};border-radius:12px;padding:12px;margin-bottom:10px;">
                    <div style="display:flex;gap:8px;align-items:center;">
                        <select id="paraphrase-style" style="flex:2;padding:9px 12px;border:1px solid ${T.border};border-radius:8px;font-size:13px;outline:none;background:${T.bg};color:${T.text};cursor:pointer;">
                        ...
                        <button id="btn-submit" style="flex:1;padding:9px 18px;border-radius:8px;font-size:13px;cursor:pointer;font-weight:500;background:linear-gradient(135deg,${T.accent},${T.accent2});color:#fff;border:none;">⟳ Выполнить</button>
                        <button id="btn-retry" style="flex:0.5;padding:9px;border-radius:8px;font-size:13px;cursor:pointer;background:${T.bg};border:1px solid ${T.border};color:${T.text};">🔄</button>
                    </div>
                </div>
                <div id="paraphrase-loading" style="display:none;text-align:center;padding:14px;color:${T.accent};font-size:13px;">⏳ Обработка...</div>
                <div id="paraphrase-result" style="display:none;">
                    <div style="background:${T.card};border:1px solid ${T.accent};border-radius:12px;padding:14px;">
                        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px;">
                            <span style="font-size:11px;color:${T.muted};font-weight:500;">РЕЗУЛЬТАТ</span>
                        ...
                        <div id="paraphrase-result-text" style="background:${T.bg};padding:12px;border-radius:10px;font-size:13px;line-height:1.7;margin-bottom:12px;white-space:pre-wrap;color:${T.text};border:1px solid ${T.border};"></div>
                        <div style="display:flex;gap:10px;">
                            <button id="btn-copy" style="flex:1;padding:10px;border-radius:8px;font-size:12px;cursor:pointer;background:${T.bg};border:1px solid ${T.border};color:${T.text};">📋 Копировать</button>
                            <button id="btn-paste" style="flex:1;padding:10px;border-radius:8px;font-size:12px;cursor:pointer;background:linear-gradient(135deg,${T.accent},${T.accent2});color:#fff;border:none;">📩 В чат</button>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div id="chat-mode" style="display:none;">
                <div style="background:${T.card};border:1px solid ${T.border};border-radius:12px;padding:14px;margin-bottom:10px;">
                    <div id="chat-messages" style="min-height:200px;max-height:300px;overflow-y:auto;font-size:13px;line-height:1.6;">
                        <div style="color:${T.muted};text-align:center;padding:40px;">Задайте вопрос ИИ 👇</div>
                    </div>
                </div>
                <div style="display:flex;gap:8px;margin-bottom:8px;">
                    <textarea id="chat-input" style="flex:1;padding:11px;border:1px solid ${T.border};border-radius:8px;font-size:13px;resize:none;outline:none;background:${T.card};color:${T.text};font-family:inherit;min-height:42px;" placeholder="Напишите сообщение..."></textarea>
                    <button id="chat-send" style="padding:11px 18px;border-radius:8px;font-size:16px;cursor:pointer;background:linear-gradient(135deg,${T.accent},${T.accent2});color:#fff;border:none;">➤</button>
                </div>
            ...
            <div id="calculator-mode" style="display:none;">
                <div style="background:${T.card};border:1px solid ${T.border};border-radius:12px;padding:12px;">
                    ...
                    <div style="display:flex;gap:8px;margin-top:8px;"><button id="calc-copy" style="flex:1;padding:8px;border-radius:8px;font-size:11px;cursor:pointer;background:transparent;border:1px solid ${T.accent};color:${T.accent};">📋 Копировать</button><button id="calc-paste" style="flex:1;padding:8px;border-radius:8px;font-size:11px;cursor:pointer;background:linear-gradient(135deg,${T.accent},${T.accent2});color:#fff;border:none;">📩 В чат</button></div>
                </div>
            </div>
            ...
            <div id="panel-templates" class="panel" style="display:none;">
                <div style="background:${T.card};border:1px solid ${T.border};border-radius:12px;padding:14px;">
                    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px;"><span style="font-size:14px;font-weight:600;">🧩 Шаблоны</span><button id="btn-add-template" style="padding:6px 16px;border-radius:8px;font-size:12px;cursor:pointer;background:linear-gradient(135deg,${T.accent},${T.accent2});color:#fff;border:none;">+ Добавить</button></div>
                    ...
                    <div id="add-template-form" style="display:none;margin-top:10px;padding:12px;background:${T.bg};border-radius:10px;border:1px solid ${T.border};">
                        ...
                        <div style="display:flex;gap:8px;"><button id="btn-save-tpl" style="flex:1;padding:8px;border-radius:8px;font-size:12px;cursor:pointer;background:${T.green};color:#fff;border:none;">💾 Сохранить</button><button id="btn-cancel-tpl" style="flex:1;padding:8px;border-radius:8px;font-size:12px;cursor:pointer;background:none;border:1px solid ${T.red};color:${T.red};">✕</button></div>
                    </div>
                </div>
            </div>
            ...
            <div id="panel-history" class="panel" style="display:none;">
                <div style="background:${T.card};border:1px solid ${T.border};border-radius:12px;padding:14px;">
                    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px;"><span style="font-size:14px;font-weight:600;">📚 История</span><button id="btn-clear-history" style="padding:4px 14px;border-radius:8px;font-size:11px;cursor:pointer;background:none;border:1px solid ${T.red};color:${T.red};">Очистить</button></div>
            ...
            <div id="panel-settings" class="panel" style="display:none;">
                <div style="background:${T.card};border:1px solid ${T.border};border-radius:12px;padding:14px;">
                    ...
                    <div style="display:flex;gap:5px;flex-wrap:wrap;margin-bottom:14px;" id="theme-selector">${Object.entries(themes).map(([k,t]) => `<button class="theme-btn" data-theme="${k}" style="flex:1;min-width:70px;padding:6px 8px;border-radius:8px;font-size:10px;cursor:pointer;border:1px solid ${settings.theme === k ? t.accent : T.border};background:${settings.theme === k ? t.accent : 'transparent'};color:${settings.theme === k ? '#fff' : T.text};">${t.name}</button>`).join('')}</div>
                    <label style="display:flex;align-items:center;gap:10px;font-size:13px;cursor:pointer;margin-bottom:10px;"><input type="checkbox" id="chk-auto-greeting-settings" ${settings.greetingEnabled?'checked':''} style="accent-color:${T.accent};width:16px;height:16px;"> Автоприветствие</label>
                    <label style="display:flex;align-items:center;gap:10px;font-size:13px;cursor:pointer;margin-bottom:14px;"><input type="checkbox" id="chk-autocopy" ${settings.autoCopy?'checked':''} style="accent-color:${T.accent};width:16px;height:16px;"> Автокопировать</label>
                    <details><summary style="font-size:12px;color:${T.muted};cursor:pointer;padding:8px 0;">⌨️ Горячие клавиши</summary>
                    <div style="margin-top:8px;padding:12px;background:${T.bg};border-radius:10px;border:1px solid ${T.border};">${['paraphrase|⟳ Перефразировать','retry|🔄 Ещё вариант','copyFromChat|📋 Из чата','pasteToChat|📩 В чат','toggleGreeting|✨ Приветствие','quickFriendly|😊 Дружелюбный','quickProfessional|💼 Деловой','quickShort|✂️ Краткий','quickPolite|🙏 Вежливый'].map(x=>{const[k,l]=x.split('|');return `<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:4px;"><span style="color:${T.muted};font-size:12px;">${l}</span><input type="text" id="hk-${k}" value="${settings.hotkeys[k]||''}" style="width:55px;padding:4px 6px;border-radius:6px;border:1px solid ${T.border};background:${T.card};color:${T.text};text-align:center;font-size:11px;outline:none;"></div>`;}).join('\n')}<button id="btn-save-hotkeys" style="margin-top:8px;width:100%;padding:8px;border-radius:8px;font-size:12px;cursor:pointer;background:linear-gradient(135deg,${T.accent},${T.accent2});color:#fff;border:none;">💾 Сохранить</button></div></details>
                    
                    <!-- ===== КНОПКА ВСЕ РЕЛИЗЫ В НАСТРОЙКАХ ===== -->
                    <button id="settings-releases-btn" style="width:100%;margin-top:12px;padding:10px;border-radius:10px;font-size:12px;cursor:pointer;background:linear-gradient(135deg,${T.accent},${T.accent2});color:#fff;border:none;font-weight:500;">📦 Все релизы на GitHub</button>
                    
                    <div style="padding-top:12px;border-top:1px solid ${T.border};text-align:center;margin-top:12px;"><span style="font-size:11px;color:${T.muted};">v${CURRENT_VERSION}</span></div>
                </div>
            </div>`;
        ...
        // ===== Все остальные обработчики (такие же, как в предыдущей версии) =====
        // Режимы, панели, чат, калькулятор и т.д.
        let calcOpen = false;
        ...
        mt.onclick = function() {
            if (currentMode === 'paraphrase') { 
                ...
                calcOpen = false; 
                mt.innerHTML = '<span>✏️</span><span>Перефразировать</span>'; 
                ...
                cam.style.display = 'none'; 
                calcOpen = false; 
                mt.innerHTML = '<span>💬</span><span>Чат</span>'; 
        ...
        // Функции (такие же, как в предыдущей версии)
        function renderStats() { 
            const now = Date.now(), st = settings.stats, total = st.paraphrased + st.copied + st.pasted; 
            document.getElementById('stats-content').innerHTML = `<div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;font-size:12px;">${[['🔄 Перефразировано', st.paraphrased],['📋 Скопировано', st.copied],['📩 Вставлено', st.pasted],['❌ Ошибок', st.errors, st.errors > 3 ? T.red : T.text],['📝 Символов', (st.totalChars || 0).toLocaleString()],['⏱ Сессия', formatTime(now - (st.sessionStart || now))],['🧠 Работает', formatTime(now - SCRIPT_START_TIME)],['🎯 Действий', total, T.accent]].map(([l, v, c]) => `<div style="display:flex;justify-content:space-between;padding:2px 0;"><span style="color:${T.muted};">${l}</span><span style="color:${c || T.text};font-weight:600;">${v}</span></div>`).join('')}</div><button id="btn-reset-stats" style="margin-top:8px;width:100%;padding:6px;border-radius:8px;font-size:11px;cursor:pointer;background:none;border:1px solid ${T.red};color:${T.red};">🔄 Сбросить</button>`; 
            document.getElementById('btn-reset-stats').onclick = function() { 
                settings.stats = { paraphrased: 0, copied: 0, pasted: 0, opened: 0, errors: 0, totalChars: 0, sessionStart: Date.now() }; 
                saveSettings(); 
                renderStats(); 
                showStatus('📊 Сброшено'); 
            }; 
        ...
        function renderTemplates() { 
            ...
            if (!settings.templates.length) { 
                l.innerHTML = '<div style="color:' + T.muted + ';font-size:12px;text-align:center;padding:15px;">Нет шаблонов</div>'; 
                return; 
            } 
            l.innerHTML = settings.templates.map((t, i) => `<div style="background:${T.bg};border-radius:8px;padding:8px 10px;margin-bottom:4px;border:1px solid ${T.border};font-size:12px;"><div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:3px;"><span class="tpl-name" data-id="${t.id}" style="color:${T.text};font-weight:500;cursor:pointer;">${t.name} ✏️</span><div style="display:flex;gap:4px;"><button class="tpl-toggle" data-i="${i}" style="background:none;border:1px solid ${t.enabled ? T.green : T.red};color:${t.enabled ? T.green : T.red};padding:1px 6px;border-radius:4px;cursor:pointer;font-size:10px;">${t.enabled ? '✅' : '⛔'}</button><button class="tpl-del" data-i="${i}" style="background:none;border:none;color:${T.red};cursor:pointer;font-size:12px;">🗑</button></div></div><div style="color:${T.muted};font-size:10px;">${t.template.substring(0, 55)}${t.template.length > 55 ? '...' : ''}</div></div>`).join(''); 
        ...
        document.getElementById('btn-add-template').onclick = function() { 
            const f = document.getElementById('add-template-form'); 
            ...
            if (f.style.display === 'block') { 
                ...
                document.getElementById('btn-save-tpl').textContent = '💾 Сохранить'; 
            } 
        };

        document.getElementById('btn-cancel-tpl').onclick = function() { document.getElementById('add-template-form').style.display = 'none'; };
        
        document.getElementById('btn-save-tpl').onclick = function() { 
            const n = document.getElementById('tpl-name').value.trim(), p = document.getElementById('tpl-prompt').value.trim(), t = document.getElementById('tpl-text').value.trim(), eid = document.getElementById('tpl-edit-id').value; 
            if (!n || !p || !t) { alert('Заполните все поля'); return; } 
            if (eid) { 
                const idx = settings.templates.findIndex(x => x.id === eid); 
                if (idx !== -1) { 
                    settings.templates[idx].name = n; 
                    settings.templates[idx].prompt = p; 
                    settings.templates[idx].template = t; 
                    showStatus('✅ Обновлён'); 
                } 
            } else { 
                settings.templates.push({ id: 'c_' + Date.now(), name: n, prompt: p, template: t, enabled: true }); 
                showStatus('✅ Добавлен'); 
            } 
            saveSettings(); 
            renderTemplates(); 
        ...
        document.getElementById('greeting-toggle').onclick = function() { 
            settings.greetingEnabled = !settings.greetingEnabled; 
            saveSettings(); 
            this.textContent = settings.greetingEnabled ? '✨ Приветствие' : '🚫 Приветствие'; 
            this.style.background = settings.greetingEnabled ? T.green : 'transparent'; 
            this.style.color = settings.greetingEnabled ? '#fff' : T.muted; 
            this.style.borderColor = settings.greetingEnabled ? T.green : T.border; 
            document.getElementById('chk-auto-greeting-settings').checked = settings.greetingEnabled; 
        };
        ...
        document.querySelectorAll('.quick-tone').forEach(b => { 
            b.onclick = function() { 
                const t = document.getElementById('paraphrase-result-text'); 
                if (t && t.textContent && !t.textContent.startsWith('❌')) { 
                    document.getElementById('paraphrase-input').value = t.textContent; 
                    document.getElementById('paraphrase-style').value = this.dataset.s; 
        ...
        document.querySelectorAll('.theme-btn').forEach(b => { 
            b.onclick = function() { 
                ...
                saveSettings(); 
                showStatus('🎨 Тема изменена'); 
                setTimeout(() => location.reload(), 500); 
            }; 
        });

        document.getElementById('btn-save-hotkeys').onclick = function() { 
            ['paraphrase', 'retry', 'copyFromChat', 'pasteToChat', 'toggleGreeting', 'quickFriendly', 'quickProfessional', 'quickShort', 'quickPolite'].forEach(f => { 
                const e = document.getElementById('hk-' + f); 
                if (e) settings.hotkeys[f] = e.value || ' '; 
            }); 
            saveSettings(); 
            showStatus('✅ Хоткеи сохранены!'); 
        };

        document.getElementById('btn-clear-history').onclick = function() { 
            history = []; 
            localStorage.setItem('ozon_crm_history', '[]'); 
            renderHistory(); 
            showStatus('🗑'); 
        };
        ...
        function renderHistory() { 
            ...
            if (!l) return; 
            if (!history.length) { 
                l.innerHTML = '<div style="color:' + T.muted + ';font-size:12px;text-align:center;padding:15px;">Пусто</div>'; 
                return; 
            } 
            l.innerHTML = history.slice(0, 10).map(i => `<div style="background:${T.bg};border-radius:8px;padding:8px 10px;margin-bottom:4px;cursor:pointer;border:1px solid ${T.border};font-size:12px;" onclick="document.getElementById('paraphrase-input').value='${i.text.replace(/'/g, "\\'").replace(/"/g, '&quot;').replace(/\n/g, '\\n')}';showStatus('✅');"><div style="display:flex;justify-content:space-between;color:${T.muted};font-size:10px;margin-bottom:3px;"><span>${i.type}</span><span>${i.date}</span></div><div style="color:${T.text};overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">${i.text.substring(0, 80)}${i.text.length > 80 ? '...' : ''}</div></div>`).join(''); 
        }
        ...
        function acm(r, t) { 
            ...
            d.style.cssText = 'margin-bottom:8px;padding:10px 12px;border-radius:10px;font-size:13px;line-height:1.5;animation:fadeIn 0.2s ease;'; 
            if (r === 'user') { 
                d.style.cssText += 'background:' + T.accent + ';color:#fff;'; 
                d.innerHTML = t; 
            } else { 
                d.style.cssText += 'background:' + T.bg + ';border:1px solid ' + T.border + ';color:' + T.text + ';'; 
        ...
        function cc() { 
            cmsg.innerHTML = '<div style="color:' + T.muted + ';text-align:center;padding:30px;">Задайте вопрос 👇</div>'; 
            chatHistory = []; 
        }
        ...
        csnd.onclick = async function() { 
            ...
            ld.style.cssText = 'color:' + T.accent + ';font-size:12px;padding:10px;'; 
            ld.textContent = '⏳ ИИ печатает...'; 
            cmsg.appendChild(ld); 
            cmsg.scrollTop = cmsg.scrollHeight; 
            try { 
                const r = await askAI([{ role: 'system', content: 'Ты полезный помощник.' }, ...chatHistory.filter(m => m.role !== 'system').slice(-10).map(m => ({ role: m.role, content: m.text })), { role: 'user', content: t }]); 
                ld.remove(); 
                acm('assistant', r); 
            } catch (e) { 
                ld.remove(); 
                acm('assistant', '❌ ' + e.message); 
            } 
        };
        ...
        console.log('✅ Ozon CRM v10.7 — ВСЁ РАБОТАЕТ!');
    }, 1500);
})();
